...
def _section_risk_assessment(data: dict[str, Any]) -> str | None:
    """Build risk assessment section with CVSS, EPSS, exploit status."""
    lines: list[str] = []
    ...
    if cvss is not None:
        ...
        vector_str = f" ({cvss_vector})" if cvss_vector else ""
        lines.append(f"CVSS v3: {cvss}{vector_str}")
    epss = data.get("epss_score")
    if epss is not None:
        percentile = data.get("epss_percentile")
        pct_str = f" (percentile: {percentile})" if percentile is not None else ""
        lines.append(f"EPSS Exploitation Probability: {epss}{pct_str}")
...
def _section_classification(data: dict[str, Any]) -> str | None:
    """Build ML + LLM classification section."""
    lines: list[str] = []
    risk_score = data.get("risk_score")
    if risk_score is not None:
        ml_conf = data.get("ml_confidence")
        conf_str = f" (confidence: {ml_conf:.0%})" if ml_conf is not None else ""
        lines.append(f"ML Risk Score: {risk_score:.2f}{conf_str}")
    llm_conf = data.get("llm_confidence")
    if llm_conf is not None:
        lines.append(f"LLM Confidence: {llm_conf:.0%}")
    return "CLASSIFICATION\n" + "\n".join(lines) if lines else None


def _section_affected_component(data: dict[str, Any]) -> str | None:
    """Build affected component section with package, version, attack vector."""
    lines: list[str] = []
    package = data.get("package")
    version = data.get("version")
    if package and package != "unknown":
        lines.append(f"Affected Package: {package}@{version or 'unknown'}")
...
def _build_rich_description(data: dict[str, Any]) -> str:
    """Build a rich plain-text description from all available pipeline data.

    Combines vulnerability metadata, enrichment data (NVD, EPSS, GitHub),
    ML/LLM classification, remediation guidance, and PDF report link
    into a structured description for the Jira ticket.
    """
    ...
    for section_fn in (
        _section_risk_assessment,
        _section_classification,
        _section_affected_component,
    ):
    ...
    references = _section_references(data)
    if references:
        sections.append(references)

    # Section 7: SIOPV metadata
    sections.append(f"Generated by SIOPV for {cve_id}")

    return "\n\n".join(sections) if sections else f"Security vulnerability {cve_id}"
...
class JiraAdapter(JiraClientPort):
    ...
    def _build_fields(self, vulnerability_data: dict[str, Any]) -> JsonDict:
        ...
        Renders all available enrichment data into a structured ticket
        description so the programmer has full context: what the
        vulnerability is, how severe, whether an exploit exists,
        what to fix, and where to find the full analysis report.
        """
        ...
        # Build rich description with all available enrichment data
        full_description = _build_rich_description(vulnerability_data)
